...
kill_app_instances() {
    ...
    PIDS=$(pgrep -f '$APP_NAME|ble_app')
    if [ -n "$PIDS" ]; then
        echo "Stopping running instances of $APP_NAME with PIDs: $PIDS..."
        kill -9 $PIDS
        sleep 2  # Wait for processes to terminate
    else
        echo "No running instances of $APP_NAME found."
    fi
}
...
cleanup_zombies() {
    ...
    ZOMBIES=$(ps -el | grep 'Z' | grep "$APP_NAME" | awk '{print $4}')
    if [ ! -z "$ZOMBIES" ]; then
...
if [ -s "$PID_FILE" ]; then
    ...
    echo "Searching for running instances of $APP_NAME..."
...
#/home/root/$APP_NAME  &> ble_logs.txt # Replace with the command to start your app
/home/root/$APP_NAME 2>&1 | ts '[%Y-%m-%d %H:%M:%S]' > /path/to/ble_app.log
